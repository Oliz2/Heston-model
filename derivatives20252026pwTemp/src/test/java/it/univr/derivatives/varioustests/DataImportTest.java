...
public class DataImportTest {
	public static void main(String[] args) {
		...
		try (FileInputStream fis = new FileInputStream(new File(filePath)); Workbook workbook = new HSSFWorkbook(fis)) { // For
			...
			if (sheet == null) {
				System.out.println("Sheet 'DAX' not found!");
				return;
			}
			 ...
			 */
			Map<LocalDate, OptionSurfaceData> marketDataDatabase = new HashMap<>();
			...
			for (int i = 3; i <= sheet.getLastRowNum(); i++) {
				 ...
				 */
				/*
				 * double[] oneMonthIVol = extractData(row, 3); // D-J double[] twoMonthsIVol =
				 * extractData(row, 10); // K-Q double[] threeMonthsIVol = extractData(row, 17);
				 * // R-X double[] sixMonthsIVol = extractData(row, 24); // Y-AE double[]
				 * twelveMonthsIVol = extractData(row, 31); // AF-AL double[] eighteenMonthsIVol
				 * = extractData(row, 38); // AM-AS double[] twentyFourMonthsIVol =
				 * extractData(row, 45); // AT-AZ
				 */	
				double[][] iVols = new double[maturities.length][strikeScaling.length];
				
				int startingIndex = 3;
				for(int j = 0; j < maturities.length; j++) {
					iVols[j] = extractData(row, startingIndex + j*7);
				}
				
				/*
				 * The risk-free curve
				 */
				double[] zeroRates = extractData(row, 52);
				...
				DiscountCurve equityForwardCurve = DiscountCurveInterpolation.createDiscountCurveFromDiscountFactors(
						...
						new double[] {valueOfUnderlying, valueOfUnderlying*Math.exp(zeroRates[0]*maturities[0]),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[1]*maturities[1])),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[2]*maturities[2])),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[3]*maturities[3])),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[4]*maturities[4])),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[5]*maturities[5])),
								valueOfUnderlying*Math.exp(valueOfUnderlying*Math.exp(zeroRates[6]*maturities[6]))
								}	/* discount factors */,
